...
public interface InterAdminDAO {
    ...
    // 5. 회원 선택 탈퇴 처리 (status를 0으로 변경)
    int deleteMember(String[] userids) throws SQLException;
    
    // 6. 상품 전체 목록 조회 (카테고리명 포함 - 페이징 없음)
    ...
    // 10. 주문 및 배송 전체 목록 페이징 처리 
    int getTotalOrderCount(Map<String, String> paraMap) throws SQLException;
    List<Map<String, String>> getOrderListWithPaging(Map<String, String> paraMap) throws SQLException;
    ...
    // 15. 상품 선택 삭제 (트랙 포함 삭제)
    int deleteProduct(String[] pnums) throws SQLException;
    
    // 16. 배송 시작 처리 (송장번호, 택배사, 받는사람 수정 및 상태변경)
